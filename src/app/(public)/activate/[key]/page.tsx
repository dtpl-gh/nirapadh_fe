...
export default function UserActivationPage({ params }: PageProps) {
    ...
    const handleActivation = async (e: React.FormEvent) => {
        ...
        try {
            setLoading(true);
            const response = await axiosInstance.post(`/auth/activate/${key}`, {
                password: password,
            });
            ...
            if (response.status === 200 || response.status === 201) {
                toast({
                    title: 'Success',
                    description: 'Account activated successfully! Redirecting to login...',
                });
                setTimeout(() => {
                    router.push('/login');
                }, 2000);
            ...
            // Error handling is centrally managed in axiosInstance interceptor
    ...
    return (
        <div className="flex min-h-screen items-center justify-center p-4">
            <Card className="sm:max-w-[800px] w-full p-8 flex flex-col justify-center rounded-lg shadow-lg">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 sm:gap-24 items-center justify-center">
                    <div className="text-center sm:text-left hidden sm:block">
                        ...
                        <h1 className="font-bold text-2xl">Activate Your Account</h1>
                        <p className="mt-4 text-gray-600">
                            Welcome to Nirapadh! Please set your new password to activate your account and gain access to your dashboard.
                        </p>
                    </div>

                    <div className="text-center sm:text-left">
                        <p className="mb-6 text-xl font-semibold sm:hidden">Activate your account</p>
                        ...
                        <form onSubmit={handleActivation} className="w-full max-w-md">
                            <div className="mb-4 text-left">
                                ...
                                <div className="relative">
                                    ...
                                    <button
                                        ...
                                        className="absolute inset-y-0 right-3 flex items-center text-gray-400 text-sm hover:text-gray-600"
                                    >
                            ...
                            <div className="mb-4 text-left">
                                ...
                                <Input
                                    ...
                                    required
                                    className={password && password === confirmPassword ? 'border-green-500' : 'border-gray-300'}
                                />
                            </div>
                            ...
                            {touchedPassword && (
                                <div className="mt-4 space-y-1 text-left">
                                    {Object.entries(VALIDATION_RULES).map(([key, { label }]) => (
                                        <div
                                            key={key}
                                            className={`flex items-center text-sm ${validations[key as keyof PasswordValidations] ? 'text-green-500' : 'text-gray-400'}`}
                                        >
                            ...
                            <Button
                                type="submit"
                                className="mt-8 w-full py-6 text-md font-semibold"
                                disabled={loading || !isAllValid || password !== confirmPassword}
                            >
                                {loading ? (
                                    <>
                                        Activating...
