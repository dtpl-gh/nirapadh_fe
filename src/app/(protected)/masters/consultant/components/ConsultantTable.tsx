...
export const ConsultantTable: React.FC<ConsultantTableProps> = ({ consultants, onDelete, onCreate, filters, onFilterChange, sort, onSortChange }) => {
    ...
    return (
        <div className="rounded-md border">
            <Table>
                <TableHeader>
                    <TableRow>
                        <TableHead>{renderColumnHeader('Consultant Name', 'user.username')}</TableHead>
                        <TableHead>{renderColumnHeader('Code', 'consultantCode')}</TableHead>
                        <TableHead>{renderColumnHeader('Specialization', 'specialization')}</TableHead>
                        <TableHead>{renderColumnHeader('Experience', 'experienceYears', false, true)}</TableHead>
                                    ...
                                    {consultant.user ? (
                                        <div className="flex items-center gap-2">
                                            <Avatar className="h-8 w-8">
                                                <AvatarImage src={consultant.user.profilePicUrl} alt={consultant.user.username} />
                                                <AvatarFallback>
                                                    {consultant.user.username.substring(0, 2).toUpperCase()}
                                                </AvatarFallback>
                                            </Avatar>
                                            <span>{consultant.user.username}</span>
                                        </div>
                                    ) : (
                                        '-'
                                    )}
                                ...
                                <TableCell className="text-right">
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end">
                                            ...
                                            <DropdownMenuItem onClick={() => router.push(`/masters/consultant/${consultant.id}`)}>
                                                <Edit className="mr-2 h-4 w-4" />
                                                Edit
                                            </DropdownMenuItem>
                                            <DropdownMenuSeparator />
                                            <DropdownMenuItem
                                                onSelect={() => setDeleteId(consultant.id)}
            ...
            <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This action cannot be undone. This will permanently delete the consultant.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
