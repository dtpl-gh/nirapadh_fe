...
const ChatSidebar = ({
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL || '/api/v1'}/admin-user-details/`);
...
const ChatArea = ({
  ...
  const fetchChatId = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_BASE_URL || '/api/v1'}/chat/api/v1/get_chat_id/?sender=${userId}&receiver=${selectedChat.user_id}`
      );
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_BASE_URL || '/api/v1'}/chat/api/v1/get_messages/${chatId}`
      );
      if (response.data.status_code === 200) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_BASE_URL || '/api/v1'}/chat/api/v1/send_message/`,
        messageData
      );
      if (response.data.status_code === 200) {
        sendMessage({
          ...
          <CardContent className="flex-grow overflow-hidden">
            <div className="overflow-y-auto h-full">
              {messages.map((msg) => (
                ...
                >
                  <div
                    className={`inline-block p-2 rounded-lg ${msg.user_ref_id === selectedChat.user_id
